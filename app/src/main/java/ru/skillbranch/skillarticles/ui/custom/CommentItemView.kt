...
class CommentItemView(context: Context): ViewGroup(context,null, 0){
    ...
    val defaultVSpace = context.dpToIntPx(8)
    val defaultHSpace = context.dpToIntPx(8)
    val avatarSize = context.dpToIntPx(40)
    val lineSize = context.dpToPx(2)
    val iconSize = context.dpToIntPx(12)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        tv_author.width = width - paddingLeft - paddingRight - avatarSize - defaultVSpace - tv_date.measuredWidth

        // все три на одной строке, поэтому usedHeight пока не меняется
        measureChild(tv_author, widthMeasureSpec, heightMeasureSpec)

        usedHeight += avatarSize + defaultHSpace

        // теперь ширину body посчитаем
        tv_body.width = width - paddingRight - paddingLeft

        measureChild(tv_body, widthMeasureSpec, heightMeasureSpec)

        usedHeight += tv_body.measuredHeight + defaultHSpace
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        if(tv_answer_to.isVisible){
            ...
            tv_author.layout(
                ...
                usedHeight + diffH,
                iv_avatar.right + tv_author.measuredWidth + defaultHSpace/2,
                usedHeight + tv_author.measuredHeight + diffH
            )
            tv_date.layout(
                iv_avatar.right + defaultHSpace/2,
                usedHeight + diffD,
                iv_avatar.right + tv_date.measuredWidth + defaultHSpace/2,
